...
- `DIM_RESPONDENT_IDENTITY` — **materialized once per Poll**, one row per response (PK `INGESTION_ID`). Resolves each response to a `HEM` (SHA-256 hashed email — the AA join key) with `HEM_SOURCE` (`payload` | `events` | NULL), and captures `PUBLICATION_NAME` (the Market) from the **same** windowed scan — the device's `[Guides-Surveys]` `publication_name`, i.e. the publication where the survey was actually shown (full names like `Miami Herald`; ~98% covered even on CRM/IAM polls whose payload `market_name` is 0% populated). The 55B-row `EVENTS_412949` scan that recovers HEM + publication from `device_id` is **windowed to the Poll's submission range and run only at refresh time** — never at report time. Refresh: `node enrichment/refresh-identity.mjs <POLL_ID> [--pad <days>] [--skip-demographics]` (`--skip-demographics` refreshes identity only, skipping the heavy AA rebuild — safe for a publication-only backfill since publication lives on this table, not the HEM-keyed demographics table). Seam (a): heavy windowed collapse in `enrichment/identity_candidates.sql`; payload-vs-events precedence + HEM normalization in the unit-tested `enrichment/resolve-identity.js`. DDL: `migration/respondent_identity_ddl.sql`.
...
- `V_SURVEY_RESPONSE_DEMOGRAPHICS` — **one row per response** (grain check in the assertion script). Left-joins identity + demographics so unresolved responses still appear with blank demographic columns; carries answers as an `ANSWERS` object (the per-Poll wide pivot is done by the pure JS row-transform, not SQL) plus `MARKET_NAME` = `COALESCE(DIM_RESPONDENT_IDENTITY.PUBLICATION_NAME, payload market_name)` (survey-interaction publication, full name; payload short-code fallback for the ~2–5% with no guide event in window). DDL: `migration/survey_response_demographics_ddl.sql`.
